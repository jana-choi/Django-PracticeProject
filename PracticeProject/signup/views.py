...
def email(request):
    ...
    try:
        ...
        msg = EmailMessage(subject="인증 코드 발송 메일", body=msg_html, from_email="id@gmail.com", bcc=[user_id])
...
def verifyView(request):
    if "user_id" in request.session.keys() and not "code" in request.session.keys():
        return redirect("main_index")
